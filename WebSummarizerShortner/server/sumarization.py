...
def processURL(url, tone, style, length, cite=None):
    ...
    if error:
        ...
        if cite:
            cite = f"Please conclude the summary with a citation in {cite} style on a new line, using the term 'reference' for APA, 'citation' for MLA, or 'bibliography' for Chicago."
        print(cite)
